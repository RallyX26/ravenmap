...
def embed_dir(root: Path, batch: int = 32) -> dict:
    """{key: (vector, relative_path)} for every image under root."""
    ...
    if cache_file.exists():
        ...
        known = {k: v for k, v in zip(z["keys"], z["vecs"])}
    ...
    for f in files:
        ...
        if len(batch_imgs) >= batch:
            _flush(vid, torch, batch_imgs, todo, keys, vecs, paths)
            batch_imgs, todo = [], []
    if batch_imgs:
        _flush(vid, torch, batch_imgs, todo, keys, vecs, paths)
...
def _flush(vid, torch, imgs, todo, keys, vecs, paths):
    with torch.no_grad():
