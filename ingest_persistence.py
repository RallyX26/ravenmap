...
def find_vehicle_fragment(
    ...
    return (
        db.merge_window_row(node_id, classification["vclass"], timestamp)
...
def merge_vehicle_fragment(
    ...
    db.bump_detections(prior["id"], timestamp, classification["conf"])
...
def insert_vehicle_sighting(record: dict[str, Any]) -> dict[str, Any]:
    ...
    reduced["id"] = db.insert_sighting(reduced)
    return reduced
...
def run_vehicle_post_insert_actions(
    ...
    evidence_crop: bytes | None, feed: Any,
) -> None:
...
def persist_vehicle_sighting(
    ...
    prior = find_vehicle_fragment(node_id, classification, timestamp, candidate)
    if prior:
        return merge_vehicle_fragment(prior, node_id, classification, timestamp)
    persisted = insert_vehicle_sighting(record)
